...
- **빌드 시스템**
  ...
  - **빌드 스크립트**
    - `npm start`: 앱 실행
    - `npm run dev`: 개발 모드 (개발자 도구 자동 열림)
    - `npm run build:win`: Windows 실행 파일 생성
    - `npm run build:mac`: macOS 실행 파일 생성
    - `npm run build:linux`: Linux 실행 파일 생성
