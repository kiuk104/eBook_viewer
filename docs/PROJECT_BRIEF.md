...
### 배포 형태
- **웹 버전**: 브라우저에서 실행 (기존 기능 유지)
- **Electron 데스크톱 앱**: 독립 실행형 애플리케이션 (v0.2.4.14 추가)
...
#### ✨ 새로운 기능
1. **링크 수정 및 삭제 기능**
   - 더블클릭: 링크 URL 및 텍스트 수정
   - Alt+클릭: 링크 삭제 (텍스트는 유지)
   - 우클릭: 컨텍스트 메뉴 (수정/복사/열기/삭제)
   - URL 유효성 검사
   - 링크 목록 조회 기능 (`getAllLinks()`)
   - 선택 영역 자동 저장 (5초 만료)
...
#### 🔧 개선
- 링크/이미지/테이블 삽입 시 선택 영역 자동 저장 로직 적용
...
1. **편집 기능 대폭 개선**
   - 편집 모드 시스템 (contentEditable 활성화)
   - 편집 히스토리 시스템 (최대 50단계)
   - 실행 취소/다시 실행 기능 (Ctrl+Z/Y)
   - 키보드 단축키 (Ctrl+S 저장, Esc 종료)
   - 토스트 알림 시스템
   - HTML → 마크다운 자동 변환 및 저장
...
1. **편집 메뉴 서브메뉴 추가**
   ...
   - Google Docs 스타일 계층형 서브메뉴
   - 복사, 잘라내기, 붙여넣기, 전체 선택 기능
   - 호버 시 자동 표시 (slideInRight 애니메이션)
   - 키보드 단축키 표시 (Ctrl+C, Ctrl+X, Ctrl+V, Ctrl+A)
...
1. **킨들 스타일 읽기 진행률 표시**
   - 좌측 하단에 위치(0-1000)와 퍼센트 실시간 표시
   - 스크롤 기반 자동 업데이트
   - 설정에서 ON/OFF 가능
   
2. **앱 정보 자동 숨김**
   - 스크롤 다운 시 중앙 하단 앱 정보 자동 숨김
   - 스크롤 업 시 다시 표시
   - 0.5초 부드러운 전환 효과
...
1. **설정 토글 크기 통일**
   - 모든 토글을 w-7 h-4 크기로 통일
   
2. **읽기 진행률 배경색 개선**
   - `theme-panel-bg` 클래스 사용
   - 테마별 자동 배경색 적용

#### 🐛 버그 수정

1. **UI 전환 실패 문제 해결**
   - 파일 업로드 후 뷰어 화면 정상 전환
   
2. **파일명 미표시 문제 해결**
   - 상단에 파일명과 크기 정보 표시
   
3. **하이라이트 복원 기능 추가**
   - TreeWalker API 사용한 정확한 위치 복원
   - 파일 재업로드 시 자동 복원

#### 🐛 버그 수정

1. **UI 전환 실패 문제 해결**
   - HTML 구조상 `mainContent`가 `page-upload` 내부에 위치
   - 부모(`page-upload`)를 숨기면 자식(`mainContent`)도 숨겨지는 문제
   - 해결: `uploadAreaContainer`만 `-translate-y-full`로 이동
...
3. **하이라이트 복원 기능 추가**
   - TreeWalker API로 정확한 텍스트 위치 계산
   - startIndex/endIndex 기반 정확한 복원
   - 기존 데이터 호환성 유지 (위치 정보 없는 하이라이트 필터링)
   - 뒤에서부터 복원하여 인덱스 변경 방지

#### 🔧 개선됨
- 디버깅 로그 개선 (UI 전환, 하이라이트 저장/복원 과정)
